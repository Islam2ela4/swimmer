...
class _LoginPageState extends BaseScreen<LoginScreen>
  ...
  Future setUserId(String id) async {
    await Appgain().addNotificationChannel({'userId': id});
  }

  Future addNotificationChannel(String email) async {
    await Appgain().addNotificationChannel({'notificationType': 'email', 'item': email});
  }
  ...
  Widget build(BuildContext context) {
    ...
    void _login(context) async {
      if (username.text.isEmpty || password.text.isEmpty) {
        ...
        await _playAnimation();
        await Provider.of<UserModel>(context, listen: false).login(username: username.text.trim(), password: password.text.trim(),
          success: (user) {
            addNotificationChannel(username.text.trim());
