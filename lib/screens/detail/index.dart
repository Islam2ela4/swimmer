...
import 'package:fstore/screens/cart/cart.dart';
            ...
            children: <Widget>[
              ...
              ListTile(
                  ...
                  onTap: () {
                    ...
                    var productParams = DynamicLinkService().productParameters(
                        productUrl: product.permalink, productId: product.id);
                    DynamicLinkService()
                        .generateFirebaseDynamicLink(productParams)
                        .then((value) {
                      Share.share(
                        value.toString(),
                        sharePositionOrigin:
...
class _ProductDetailPageState extends State<ProductDetailScreen> {
  ...
  Widget build(BuildContext context) {
    var productDetail = Provider.of<AppModel>(context).appConfig['Setting']['ProductDetail'];

    var layoutType = productDetail ?? (kProductDetail['layout'] ?? 'simpleType');

    var layout = Services().widget.renderDetailScreen(context, product, layoutType);
