...
class _OrderedSuccessState extends BaseScreen<OrderedSuccess> {
  ...
  Future cancelfireAutomatorWithPersonalization() async {
    await Appgain().cancelfireAutomatorWithPersonalization({
      'triggerPointName': "abandon",
    }); //here add to triggerPointName

    print("cancelfireAutomatorWithPersonalization");
  }

  Future customEvents(String type, String action) async {
    var data = {'type': type, 'action': action};
    await Appgain().logEvent(data); //here add to triggerPointName
  }

  Future purshase(double amount) async {
  ...
  void didChangeDependencies() async {
    await purshase(widget.order.total);
    await cancelfireAutomatorWithPersonalization();
    // await customEvents("Purchase", "Purchase");
