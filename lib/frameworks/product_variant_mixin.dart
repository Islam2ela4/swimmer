...
mixin ProductVariantMixin {
  ...
  List<Widget> makeBuyButtonWidget(
    ...
    final isVariationLoading =
        product.type == 'variable' && productVariation == null;
            ...
            Expanded(
              child: GestureDetector(
                onTap: () async {
                  ...
                  await fireAutomatorWithPersonalization(product.name);
                  await customEvents("Add to Cart", product.name);
                },
                child: Container(
                  height: 44,
                  decoration: BoxDecoration(
  ...
  Future fireAutomatorWithPersonalization(String productName) async {
    await Appgain().fireAutomatorWithPersonalization({
      'triggerPointName': "abandon",
      'personalizationMap': {"item": productName}
    }); //here add to triggerPointName

    print("fireAutomatorWithPersonalization");
  }
