...
class ShopifyWidget extends BaseFrameworks
  ...
  Future<void> applyCoupon(
    ...
    try {
      ...
      final totalPriceBeforeApplyCoupon = cartModel.getTotal();
      ...
      if (checkout != null) {
        ...
        // response coupon
        final totalPriceAfterApplyCoupon = cartModel.getTotal();

        if (totalPriceAfterApplyCoupon == totalPriceBeforeApplyCoupon) {
          final checkout =
              await ShopifyApi().removeCoupon(cartModel.checkout.id);
        ...
        var coupon = Coupon.fromShopify({
          'discount': totalPriceBeforeApplyCoupon - totalPriceAfterApplyCoupon,
          'code': code,
        });

        success(Discount(coupon: coupon));
  ...
  Future<void> removeCoupon(context) async {
    final cartModel = Provider.of<CartModel>(context, listen: false);
    try {
      final checkout = await ShopifyApi().removeCoupon(cartModel.checkout.id);
  ...
  void updateUserInfo(
    ...
    Services().api.updateUserInfo(params, loggedInUser.cookie).then((value) {
      ...
      onSuccess(params != null ? User.fromShopifyJson(params, loggedInUser.cookie) : loggedInUser);
    }).catchError((e) {
      onError(e.toString());
    });
  }
