...
//          'sortKey': sortKey,
  ...
  Future<Product> getProductBySlug(slug, {lang}) async {
    printLog('::::request getProductBySlug $slug');

    const nRepositories = 50;
    final options = QueryOptions(
      documentNode: gql(ShopifyQuery.getProductBySlug),
      variables: <String, dynamic>{'nRepositories': nRepositories, 'sku': slug},
    );
    final result = await client.query(options);
  ...
  Future<CheckoutCart> removeCoupon(String checkoutId) async {
    try {
      final options = MutationOptions(
        documentNode: gql(ShopifyQuery.removeCoupon),
        variables: {
          'checkoutId': checkoutId,
        },
      );
  ...
  @override
  Future<PagingResponse<Order>> getMyOrders({User user, dynamic cursor}) async {
    try {
      const nRepositories = 50;
      final options = QueryOptions(
  ...
  Future<String> submitForgotPassword({
    ...
    final options = MutationOptions(
      documentNode: gql(ShopifyQuery.resetPassword),
      variables: {
        'email': data['email'],
      },
    );
